...
class AddrProvider(DataProvider):

    def __init__(self, args, need_shuffle=True):
        self.addr_codes_path = "../data/addr/codes_3digits.csv"
        self.d3_table_path = "../data/addr/d3_table.pickle"
        self.d3table = self.load_d3table()
        self.class_map = list(set(self.d3table['code']))
        self.total_class = len(self.class_map)
        super(AddrProvider, self).__init__(args, need_shuffle)
        self.load_all()

    def load_d3table(self):
        return pd.read_csv(self.addr_codes_path, engine='python',
